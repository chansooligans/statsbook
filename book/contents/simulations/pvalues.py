...
def sim(pop, sample_size = 40):
    ...
    sample_var = np.var(sample)
    test_statistic = (sample_mean - 0) / np.sqrt(sample_var/(sample_size-1))
    pvalue = 1-scipy.stats.norm.cdf(test_statistic, 0, 1)
...
# %% [markdown]
"""
for this data, using an alpha=0.05, about 71% of test statistics are statistically significantly 
different from zero (larger than 1.645)
"""
...
statistics larger than 1.645 (black vertical line) equivalently, to reject all pvalues less than 0.05. 
...
statistics less than 1.645  
- the shaded area in red represents a percentage of the red distribution (we computed the area under the 
red distribution ABOVE 1.645 earlier, so this is just 1 minus that value): 1 - .843 = 15.7%
"""
...
def animate(frame_number):
    ...
    sns.distplot(std_normal, color="blue", hist=False, ax=ax1)
    sns.distplot(results["test_statistic"], color="red", hist=False, ax=ax1)
    l1 = ax1.lines[0]
    l2 = ax1.lines[1]
    x2 = l2.get_xydata()[:,0]
    y2 = l2.get_xydata()[:,1]
    ...
    ax1.fill_between(x2[x2>teststat],y2[x2>teststat], color="red", alpha=0.3)
